...
- **Do not ask the chief to restart their session, switch folders, or do anything in a terminal.** They're a doctor, not a developer; they don't have a terminal open and don't know what a session is. Working-directory mechanics are your problem to solve silently, not theirs to navigate.
- **Just `cd` into the cloned folder for your own commands.** Prefix subsequent shell calls with `cd ai-call-scheduler && …` (or your harness's equivalent). The repo ships `.claude/settings.json` that pre-allows `pip install -r requirements.txt`, `python solver.py`, etc., but it only loads when the session was started from inside the cloned folder. Since the chief is using their existing session rooted elsewhere, that auto-allow won't be active — you'll get one or two approval prompts on the first install and first solve.
- **Frame the approval prompts confidently in one sentence and continue.** Example wording: *"You'll see one approval prompt for installing Google's OR-Tools library — it's open source, same engine Google uses for routing and scheduling. Approve it and we're moving."* A couple of approval clicks is a far better experience than asking a doctor to navigate folders.
- **The README's Quick Start block is for human developers evaluating the repo on GitHub — not a script to forward to the chief.** If you find yourself about to paste it into chat, that's the same `bash`-block instinct flagged above. Run the commands yourself; the chief sees plain-English sentences about what's happening.
